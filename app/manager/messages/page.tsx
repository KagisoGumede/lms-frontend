...
export default function ManagerMessages() {
  ...
  useEffect(() => { fetchInbox(); fetchContacts(); }, [user]);
  ...
  const roleBadge = (role: string) =>
    ...
    role === 'MANAGER' ? 'bg-blue-50 text-blue-700' : 'bg-emerald-50 text-emerald-700';

  const filteredInbox = inbox.filter(c => c.contactName?.toLowerCase().includes(search.toLowerCase()));
  const filteredContacts = contacts.filter(c =>
    `${c.firstName} ${c.surname} ${c.department}`.toLowerCase().includes(contactSearch.toLowerCase())
  );
  const selectedName = selectedContact?.contactName ?? `${selectedContact?.firstName} ${selectedContact?.surname}`;
  const selectedRole = selectedContact?.contactRole ?? selectedContact?.role;

  if (!user) return null;

  return (
    <ManagerLayout title="Messages">
                    ...
                    <div key={m.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                      ...
                      <div className={`max-w-xs lg:max-w-md flex flex-col ${isMine ? 'items-end' : 'items-start'}`}>
                        <div className={`px-4 py-2.5 rounded-2xl text-sm ${
                          isMine ? 'bg-[#0f1f3d] text-white rounded-br-sm' : 'bg-gray-100 text-gray-800 rounded-bl-sm'
                        }`}>
                          {m.content}
                        </div>
            ...
            <div className="px-4 py-3 border-t border-gray-100">
              <div className="flex gap-2 items-end">
                <textarea value={newMessage}
                  ...
                  placeholder={`Message ${selectedName}...`} rows={1}
