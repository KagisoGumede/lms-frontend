...
export default function ManagerApplyLeavePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // silently fail on balance load — not critical
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user) return;
    if (!formData.leaveType) { setError('Please select a leave type.'); return; }
    if (!formData.startDate || !formData.endDate) { setError('Please select start and end dates.'); return; }
    if (duration <= 0) { setError('End date must be after start date.'); return; }
    ...
    try {
      ...
      if (file) await leaveAPI.uploadDocument(res.data.id, file);
      setSuccess('Leave request submitted successfully.');
      setTimeout(() => router.push('/manager/my-leaves'), 1500);
    } catch {
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              {leaveTypesLoading ? (
                <div className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-gray-400 text-sm bg-gray-50">
                  Loading leave types...
                </div>
              ) : leaveTypes.length === 0 ? (
                <div className="w-full px-3 py-2.5 border border-amber-200 rounded-lg bg-amber-50 text-amber-700 text-sm">
                  No leave types available. Add them in Settings.
                </div>
              ) : (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className={`rounded-xl p-4 text-center flex flex-col justify-center ${
                willExceed ? 'bg-red-600' : 'bg-[#0f1f3d]'
              }`}>
            ...
            {willExceed && (
              <div className="p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-xs font-semibold">
                This request exceeds your remaining balance by {duration - selectedBalance.remaining} day
                {duration - selectedBalance.remaining > 1 ? 's' : ''}.
              </div>
            )}
