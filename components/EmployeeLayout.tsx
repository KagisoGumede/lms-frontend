...
export default function EmployeeLayout({ children, title }: EmployeeLayoutProps) {
  ...
  useEffect(() => {
    if (!isAuthenticated || user?.role !== 'employee') router.push('/');
  ...
  { name: 'Apply for Leave',   path: '/employee/apply-leave' },
  { name: 'My Leave Requests', path: '/employee/my-leaves' },
  { name: 'Leave Calendar',    path: '/employee/calendar' },
  { name: 'My Team',           path: '/employee/my-team' },
    ...
    <aside className="w-64 bg-[#0f1f3d] text-white flex flex-col h-full shadow-xl">
      ...
      <div className="p-4 border-t border-white/10">
        <div className="flex items-center gap-3 mb-3 px-1 cursor-pointer hover:opacity-80 transition"
          onClick={() => { router.push('/employee/profile'); setSidebarOpen(false); }}>
          ...
          <div className="overflow-hidden">
            ...
            <p className="text-xs text-blue-300">Employee</p>
          </div>
        </div>
